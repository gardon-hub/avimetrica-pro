...
function HistogramaSVG({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" role="img"
      aria-label={`Histograma de ${hist.bins.length} clases con curva normal superpuesta`}>
      ...
      {hist.bins.map((b, i) => {
        ...
        return (
          <rect key={i} x={x + 0.5} y={y} width={w} height={Math.max(toY(0) - y, 0)}
            ...
            <title>{`${b.x0.toFixed(1)}–${b.x1.toFixed(1)} ${unidad}: ${b.count} (${b.percent.toFixed(1)}%)`}</title>
          </rect>
        );
      })}
      ...
      <text x={W / 2} y={H - 4} textAnchor="middle" fontSize={11} fontWeight="bold" fill="#444">
        {`Histograma (${hist.bins.length} clases)${curva ? ' con curva normal' : ''} — ${unidad}`}
      </text>
    </svg>
  );
}
...
export function DescriptivePanel({ valores, variable }: { valores: number[]; variable: VariableDefinition }) {
    ...
    return (
      <div className="bg-card rounded-lg border shadow-sm p-4 mb-4">
        <p className="text-xs text-muted-foreground text-center">
          Registra valores para ver el análisis estadístico.
        </p>
      </div>
    );
  }
  ...
  const filas: Array<[string, string]> = [
    ...
    ['Desv. estándar (n−1)', `${f(d.sdSample)} ${u}`],
    ['Coef. de variación', `${f(d.cv, 2)} %`],
    ['Error estándar', `${f(d.sem)} ${u}`],
    ...
    ['Curtosis (G2)', d.kurtosis === null ? '—' : f(d.kurtosis, 3)],
  ...
  return (
    <div className="bg-card rounded-lg border shadow-sm p-3 sm:p-4 mb-4">
      <h2 className="text-sm font-bold uppercase tracking-wide text-muted-foreground mb-3 flex items-center gap-1.5">
        <Sigma className="h-4 w-4" /> Análisis estadístico
      </h2>
      ...
      <div className="mt-3 text-[11px] space-y-1 bg-muted/50 rounded-md p-2.5">
        {sw ? (
          <div>
            <b>Normalidad (Shapiro-Wilk):</b> W = {sw.W.toFixed(4)}, valor p ={' '}
            {sw.pValue < 0.0001 ? '< 0.0001' : sw.pValue.toFixed(4)} —{' '}
            {sw.pValue >= 0.05
              ? 'los datos son compatibles con una distribución normal (no lo demuestra).'
              : 'los datos se desvían de la normal; revisa el histograma.'}
          </div>
        ) : (
          <div className="text-muted-foreground">Normalidad: se requieren al menos 3 valores con variabilidad.</div>
        )}
        <div>
          <b>Posibles atípicos:</b>{' '}
          {out.flags.length === 0
            ? 'ninguno marcado.'
            : `${out.flags.length} marcado(s) — ${out.flags.map((x) => `#${x.index + 1} (${f(x.value)})`).join(', ')}. Se señalan para revisión; no se eliminan.`}
        </div>
        {d.n < 30 && (
          <div className="text-amber-700">
            ⚠️ Con n = {d.n} (&lt; 30) las estimaciones tienen amplia incertidumbre.
          </div>
        )}
      </div>
    </div>
